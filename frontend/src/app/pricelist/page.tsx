...
const PricePage = () => {
    ...
    return (
        <div className='text-white'>
            ...
            <div className="absolute top-0 left-0 w-full h-full flex flex-col space-y-10 justify-center items-center">
                <div className="w-7/12 text-start">
                    <h1 className="text-4xl font-bold mb-4">Our Pricelist</h1>
                    <p className="text-base">Discover our competitive pricing for top-notch photography services.</p>
                    <p> We offer a range of packages to suit your needs and budget.</p>
                </div>
                <div className="flex gap-3 w-7/12">
                    {
                        priceList.map((item, index) => (
                            <Card key={index} className='p-5 w-4/12 bg-white bg-opacity-10 backdrop-blur-lg text-white rounded-lg shadow-lg'>
                                ...
                                <div className="price mb-4">
                                    {
                                        item.isDiscount ? (
                                            <div className="flex items-baseline gap-2">
                                                <span className="text-xl font-bold">Rp. {hiddenNumber(item.discountPrice)}</span>
                                                <span className="text-sm text-green-500">{item.discountPercentage}% off</span>
                                            </div>
                                        ) : (
                                            <span className="text-xl font-bold">Rp. {hiddenNumber(item.originalPrice)}</span>
                                        )
                                    }
                                </div>
